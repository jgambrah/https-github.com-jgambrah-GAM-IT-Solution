      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {apps.map((app, index) => (
            <motion.div
              ...
              className="group relative bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100 flex flex-col"
            >
              ...
              <div className="p-8 flex-grow">
                ...
                <p className="text-indigo-600 font-medium text-sm mb-4 uppercase tracking-wider">{app.tagline}</p>
              ...
              <div className="p-8 pt-0 flex flex-col gap-3">
                {app.external ? (
                  <a 
                    href={app.link}
                    className="inline-flex items-center justify-center w-full py-3 px-6 rounded-xl bg-gray-900 text-white font-semibold hover:bg-gray-800 transition-colors"
                  >
                    Access App
                  </a>
                ) : (
                  <Link 
                    to={app.link}
                    className="inline-flex items-center justify-center w-full py-3 px-6 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors"
                  >
                    View Details
                  </Link>
                )}
        ...
        <div className="mt-20">
          <div className="text-center mb-12">
            <h3 className="text-2xl font-bold text-gray-900">Future Horizons</h3>
            <p className="text-gray-600">Engineering the next generation of sector-specific intelligence.</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <motion.div
                ...
                className="p-6 bg-white rounded-2xl border border-dashed border-gray-200 flex flex-col items-center justify-center text-center group hover:border-indigo-300 transition-colors"
              >
                <span className="text-3xl mb-3 group-hover:scale-110 transition-transform">{future.icon}</span>
                <span className="font-semibold text-gray-900">{future.name}</span>
                <span className="text-xs text-indigo-600 mt-1 font-medium uppercase tracking-tighter">In Development</span>
              </motion.div>
            ))}
          </div>
        </div>
